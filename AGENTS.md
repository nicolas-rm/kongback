...
- Documenta observaciones de flujo cuando un dato se administra desde otro modulo o endpoint dedicado. Ejemplos: combustibles de estacion via `station-fuels`, accesos de usuario via `/users/:id/access`, conductor de vehiculo via `/vehicles/:id/driver`, tarjeta asignada via endpoints dedicados de `cards`.
- No trates `openapi/meta.json`, nombres de archivos MDX ni tablas de endpoints como mapa de navegacion del frontend. La documentacion puede tener paginas para endpoints tecnicos que no deben existir como menu o pantalla CRUD; cuando aplique, declaralo en `Frontend Guidelines` y en `Notas para frontend`.

...
if (!prismaConnectionString) {
    throw new Error('DIRECT_URL o DATABASE_URL es requerido para ejecutar prisma/seed-minimal.ts');
}
...
const ROLE_DEFINITIONS = [
    ...
    },
    {
        ...
        extraPermissionCodes: ['fuels.read-list', 'fuels.read-one', 'sub-companies.read-list', 'sub-companies.read-one', 'users.read-list', 'users.read-one', 'cardholders.read-list'],
    },
...
function getRequiredEnv(name: string): string {
    ...
    if (!value) {
        throw new Error(`${name} es requerido para ejecutar prisma/seed-minimal.ts`);
    }

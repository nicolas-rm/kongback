...
export class StationsRepository {
    ...
    findById(id: string, scope?: CompanyScope) {
        return this.prisma.station.findFirst({ where: { id, subCompany: subCompanyScopeWhere(scope) }, select: this.select() });
    }

    update(id: string, data: Prisma.StationUncheckedUpdateInput, address?: AddressData, scope?: CompanyScope) {
        return this.prisma.$transaction(async (tx) => {
            const current = await tx.station.findFirst({ where: { id, subCompany: subCompanyScopeWhere(scope) }, select: { id: true, addressId: true } });
            ...
            return tx.station.findFirst({ where: { id: current.id, subCompany: subCompanyScopeWhere(scope) }, select: this.select() });
        });
    }

    deactivate(id: string, scope?: CompanyScope) {
        return this.prisma.$transaction(async (tx) => {
            const result = await tx.station.updateMany({ where: { id, subCompany: subCompanyScopeWhere(scope) }, data: { status: Status.inactive } });
            if (result.count === 0) return null;
            return tx.station.findFirst({ where: { id, subCompany: subCompanyScopeWhere(scope) }, select: this.select() });
        });
    }
